...
;;;;<br><br> 
;;;; This script processes logfiles produced by prog1.rs (or prog0.rs), and it uses 
;;;; a heuristic algorithm of my devising to detect fire events from the temperature readings.
;;;; That is, it aggregates individual temperature readings to estimate fire positions.
;;;; It then compares these detections vs. ground truth and detects the lag.

;;;; It depends on the firelightning sim producing specific log messages.
;;;; Also expected format of SOCRETURN values is a tuple of: <NodeID, Clock, Temp>
...
(define (main) 
	 ...
	 (let ([nodes (map (lambda (id) (assq id nodes)) ids)])
       ...
       ;; This uses the returned data to maintain a rough estimate of where (and whether) the fire is.
