...
// In this one I tried my own hash function.
// Did worse.
...
//   }
   ...
   static size_t generic_hash(const unsigned char* ptr, int size) {
     size_t hash = 5381;
     int c;
...
class Iter_s_2 : public WSBox {
  ...
  bool iterate(uint32_t portnum, void* datum) {
    ...
    unsigned char* str = (unsigned char*) sym_6.c_str();
    size_t hsh = WSPrim::generic_hash(str, sym_6.size());
    
    // This manual hack assumes no collisions!!
    //    printf("HASH: %d\n", hsh);

    wsfloat_t t_7 = (pattmp_5.fld2);
    wsint_t vol_8 = (pattmp_5.fld3);
    wsfloat_t price_9 = (pattmp_5.fld4);
    if (WSPrim::wsnot((ht_3)[hsh])) {
      (ht_3)[hsh] = 1.0;
    } else {
      tuptyp_12();
    }
    if (vol_8 == -1) {
      (ht_3)[hsh] = ((ht_3)[hsh] * price_9);
    } else {
      emit((tuptyp_10)tuptyp_10(sym_6, t_7, vol_8, (price_9 * (ht_3)[hsh])));
    }
    return FALSE;
  } 
} 
