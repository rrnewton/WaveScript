...
invocations.  This is introduced with the sub-keyword {\cd state}.
...
({\cd :=}) is different than the operator used for declaring new
variables ({\cd =}).  ({\ws} also has {\cd +=}, {\cd -=} etc for use
...
every three zeroes.  The {\cd merge} operator combines two streams of
...
signatures.  Also see the libraries in {\cd wavescript/lib} for
