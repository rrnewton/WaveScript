...
(define (extract-documentation-from-scheme-file scheme-file)
  (lexical-to-syntactical-comments! scheme-file temp-output-file)
  ; hereby temp-output-file is defined
  (let ((res (extract-relevant-documentation temp-output-file)))
      (if delete-comment-file? (delete-file temp-output-file))
    res))
  ...
  (let* ((lgt (string-length attr-desc-string))
    ...
    (let ((attr-name (substring attr-desc-string start-pos pos-1))
      ...
      (if (not (or (equal? attr-status "implied") (equal? attr-status "required")))
          (display-warning "Attribute" (string-append attr-name ":") "Use either required or implied instead of" attr-status))
