...
(library (ws passes static_elaborate static-elaborate)
  (export static-elaborate
...
;	(floatToDouble (lambda (x) x))
	...
	(floatToComplex ,(lambda (env f) `(quote ,(+ f (make-rectangular 0.0 0.0)))))
	...
	(if entry
		...
		`(quote ,(reg:top-level-eval `(,(cadr entry) ,@(map (lambda (a) `(quote ,a)) args)))))
...
(define-testing test-static-elaborate 
  (default-unit-tester
