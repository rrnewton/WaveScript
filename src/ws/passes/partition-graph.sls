...
(library (ws passes partition-graph)
  (export 
...
(define (exhaustive-partition-search heuristic part)
  (match part
    [(,input-language 
     ...
     (define ____ (begin (printf "Beginning search")(flush-output-port (current-output-port))))
    ...
    (+ 0     ;; HACK FIXME!!! ADDING to it temporarily to artificially stress the LP solver:
	 ...
	 (let  ([entry (assq 'measured-cycles (hashtab-get annot-table vr))])
	   ...
	   (if entry
		 ...
		 (exact (floor (* frac cpu-granularity))))
		 ...
		 (if entry
			 ...
			 (printf "  TIME ~s ~s ELAPSED ~s RATIO ~s\n" vr
				 (bench-stats-cpu-time (caddr entry)) elapsed-vtime (inexact frac))
			 (exact (floor (* frac cpu-granularity)))))
		     default-vert-weight))
	       )))))
  ;;------------------------------
...
(define test-partition-graph 
  (default-unit-tester "Partitioning Node/Server programs" 
    `(
      )))
