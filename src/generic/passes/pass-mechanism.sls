...
(library (ws passes pass-mechanism) 
  (export  define-pass fuse-passes/disjoint fuse-passes
	   test-pass-mechanism
	   ;P1 P2 F G
	   )
  (import  (rnrs (6))
      ...
      [(_ name clauses ...)
       (let ([gen-code 
		...
		(define output-language 
		  (datum->syntax #'name
		   (string->symbol 
		    (string-append (symbol->string (syntax->datum #'name)) "-language"))))
...
(define-testing test-pass-mechanism 
  (default-unit-tester "pass-mechanism: Machinery for building compiler passes."
    '(
