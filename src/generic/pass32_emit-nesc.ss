...
(define emit-nesc
  (let ()
    ...
    (define tokname 
      (match-lambda ((tok ,str ,id))
	(list->string (filter (lambda (c) (memq c acceptable_chars)) (string->list str)))))
    ...
    (define (tok_id t)
      (string-append "AM_" (upcase (tokname t))))
...
(define (process_stmt indent tokargs e) 
  (match e
