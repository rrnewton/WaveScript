...
;				      )]
    ...
    (notlist
         [(NUM)    (wrap $1-start-pos $1-end-pos $1)]  
         [(VAR)    (wrap $1-start-pos $1-end-pos $1)]
         [(CHAR)   (wrap $1-start-pos $1-end-pos $1)]
         [(STRING) (wrap $1-start-pos $1-end-pos $1)]
         [(true)   (wrap $1-start-pos $1-end-pos ''#t)]
	 [(false)  (wrap $1-start-pos $1-end-pos ''#f)]

         [(tuple)  (wrap $1-start-pos $1)]
	 
	 ;; Deconstructing sum types with pattern matching:
	 [(match exp LeftBrace matchcases RightBrace) 
	  (wrap $1-start-pos $5-end-pos `(match ,$2))]
         
;         [(VAR DOT DOT LeftSqrBrk NUM RightSqrBrk) (prec DOTBRK) `(seg-get ,$4 ,$1)]
         [(VAR LeftSqrBrk LeftSqrBrk exp RightSqrBrk RightSqrBrk) 
	  (wrap $1-start-pos $6-end-pos
	   `(seg-get ,$1 ,$4))]
;         [(VAR BAR exp BAR)  `(seg-get ,$1 ,$3)]
	 ...
	 [(exp MAGICAPPLYSEP VAR LeftParen expls RightParen) 
	  (wrap $1-start-pos $6-end-pos 
		`(app ,$3 ,$1 . ,$5))]
         [(exp MAGICAPPLYSEP VAR) 
	  (wrap $1-start-pos $3-end-pos 
		`(app ,$3 ,$1))]
	 ...
	 [(exp $ exp) 
	  (wrap $1-start-pos $3-end-pos
		`(app ,$1 ,$3))] ;; Note, every binop adds a shift/reduce conflict currently¢

	 ;; Extended dot syntax, adds three more shift-reduce conflicts;
	 [(exp DOT VAR LeftParen expls RightParen) 
	  (wrap $1-start-pos $6-end-pos
		`(app ,$3 ,$1 . ,$5))]
	 ;; Basic dot syntax:
	 ...
	 [(exp DOTSTREAM expls+ RightParen) 
	  (wrap $1-start-pos $4-end-pos
		`(dot-project ,$3 ,$1))]
	 ...
	 ;; Shorthands:
         [(VAR += exp) (wrap $1-start-pos $3-end-pos `(set! ,$1 (+ ,$1 ,$3)))]
         [(VAR -= exp) (wrap $1-start-pos $3-end-pos `(set! ,$1 (- ,$1 ,$3)))]
         [(VAR *= exp) (wrap $1-start-pos $3-end-pos `(set! ,$1 (* ,$1 ,$3)))]

	 ;; Operators that are simple are straightforward.
         [(VAR LeftParen expls RightParen) (wrap $1-start-pos $4-end-pos `(app ,$1 ,@$3))]
	 ;; This is a keyword, but it's also a prim name:
         [(static LeftParen expls RightParen) (wrap $1-start-pos $4-end-pos `(app static ,@$3))]

	 ;; Other operators must be wrapped in parens:
         [(LeftParen exp RightParen LeftParen expls RightParen)
	  (wrap $1-start-pos $6-end-pos `(app ,$2 ,@$5))]

	 ;; Array references/assignments:
         [(VAR LeftSqrBrk notlist RightSqrBrk) (prec APP) 
	  (wrap $1-start-pos $4-end-pos `(Array:ref ,$1 ,$3))]
         [(LeftParen exp RightParen LeftSqrBrk notlist RightSqrBrk) 
	  (wrap $1-start-pos $6-end-pos `(Array:ref ,$2 ,$5))]
                  
         ;; Begin/end:
         [(LeftBrace stmts RightBrace) (wrap $1-start-pos $3-end-pos (make-begin $2))]
	         
         ;; Anonymous functions:
         [(fun LeftParen formals RightParen exp) (prec else) 
	  (wrap $1-start-pos $5-end-pos `(lambda ,$3 ,$5))]

         ;; Iterators:
         [(iter iterbinder LeftBrace stmts RightBrace) 
	  (wrap $1-start-pos $5-end-pos
		`(,$1 (lambda (,(car $2) ,VIRTQUEUE)
			,(make-begin (append $4 (list VIRTQUEUE)))) ,(cadr $2)))]
