...
#define WSFIFOPUT(ff, val, ty) { onestage_WSFIFOPUT(& (ff)->buffer, val, ty); ff->pending++; }
//#define WSFIFOPUT(ff, val, ty) {  }

void* wsfifoget(wsfifo* ff) { return onestage_wsfifoget(& ff->buffer); }
...
int wsfifosize(wsfifo* ff) {
  return onestage_wsfifosize(&ff->buffer) 
