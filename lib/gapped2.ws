...
uniontype Gapped t = Gap (Int64 * Int) | Seg (Sigseg t);
//type GappedStream t = Stream (Gapped t);
...
// Gapped:Internal:syncN_aux                :: (Stream 'hci, List (Stream 'hce), Bool) -> Stream (List (Sigseg 'hwj));

// syncN_aux :: (CtrlStrm, List (Stream (Sum Gapped t)), Bool) -> SLS t;
 ...
 fun (ctrl, strms, del) {
  ...
  iterate sum in union2(ctrl, unionList(strms)) {
    state {
      accs = Array:make(List:length(strms), nullseg);
      requests = [];
    }
...
fun rewindow(sig,newwidth,gap)
  ...
  //syncN_no_delete :: (CtrlStrm, List (GappedStream t)) -> SLS t;
syncN_no_delete = fun (ctrl, strms) { Gapped:Internal:syncN_aux(ctrl, strms, false) }
...
ctrl :: CtrlStrm;
ctrl = iterate _ in timer(1.0) {
  state { cnt = 0`to64; b = true; }
  if b 
  then emit (true,  cnt, cnt+1`to64)
  else emit (false, cnt, cnt+1`to64);
  cnt += 2`to64;
  b := not(b);
}
...
//gapped :: GappedStream Int;
gapped = markgaps(g);

BASE <- syncN_no_delete(ctrl, [gapped]);
